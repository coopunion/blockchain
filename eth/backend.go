...
func New(stack *node.Node, config *Config) (*Ethereum, error) {
	...
	// set state fn if consensus engine is themis.
	if themisEngine, ok := eth.engine.(*themis.Themis); ok {
		themisEngine.SetStateFn(eth.blockchain.StateAt)
	}
...
func CreateConsensusEngine(stack *node.Node, chainConfig *params.ChainConfig, config *ethash.Config, notify []string, noverify bool, db ethdb.Database) consensus.Engine {
	...
	if chainConfig.Themis != nil {
		return themis.New(chainConfig, db)
	}
...
func (s *Ethereum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		if themis, ok := s.engine.(*themis.Themis); ok {
			wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
			if wallet == nil || err != nil {
				log.Error("Etherbase account unavailable locally", "err", err)
				return fmt.Errorf("signer missing: %v", err)
			}
			themis.Authorize(eb, wallet.SignData)
		}
